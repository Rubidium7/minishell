...
// form_tree.c
t_ast			*form_tree(t_token *start, t_ast *up, int end_index,
					int *error_index);
// form_pipeline.c
t_pipeline		*form_pipeline(t_token *head_token, int end_index,
					int *error_index);
//form_command.c
...
// REDIRECTIONS
// redirections.c
void			open_redirections(t_command *new, t_pipeline *pipeline,
					t_heredoc *heredoc, t_shell *core);
// heredoc.c
...
t_bool			heredoc_input(int fd, char *limiter, t_bool expand,
					t_shell *core);
// find_token_nodes.c
...
// errors.c
void			print_generic_error(const char *cmd, const char *arg,
					const char *msg);
...
int				finalise_pipes_and_children(int **pipes, pid_t *children,
					int len);
